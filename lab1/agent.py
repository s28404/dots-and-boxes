...
class DotsAndBoxesAI(TwoPlayerGame):
    ...
    def make_move(self, move):
        ...
        # t = line type, r = row, c = column
        t, r, c = move.split(",")
        r, c = int(r), int(c)

        # Save state before move for unmake_move
        # row[:] creates a shallow copy of each row, it is needed to avoid reference issues
        # like for example self.h_lines = old_h_lines would just copy the reference 
        # what would lead to both variables changing when one is changed
...
def find_best_move(game_state, depth):
    ...
    """
    # Use deepcopy to avoid modifying the original game state
    wrapper = DotsAndBoxesAI(copy.deepcopy(game_state))
    # move_str is in format "t,r,c"
